...
#data = pd.read_csv('final_data_train.csv', header = 0, names = ['Query', 'Description'])
data = pd.read_csv('own_data.csv', header = 0, names = ['Query', 'Description'])
    
#Split the dataset to Training and Test subsets (95/05)
train, test = train_test_split(data, test_size = 0.05, random_state = 42) #random_state = None
...
def model_pred(model, job_des = x_test):
